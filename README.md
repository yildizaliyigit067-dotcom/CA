...
- `03_sensitivity.ipynb`: Tek hücrelik başlangıç pertürbasyonlarının beş kuralda (0, 4, 30, 90, 110) yayılması; fark alanı, normalize Hamming uzaklığı D(t) ve pertürbasyon yarıçapı R(t) ölçümleri, nedensel ışık konisiyle karşılaştırma, 10 tohumluk tekrar deneyi ve Kural 90'ın doğrusallığı üzerine isteğe bağlı araştırma.
- `AI_NOTES.md`: 1., 2. ve 3. haftanın AI'dan hangi konularda yardım alındığının kaydı.

## Ana fikir

Basit yerel etkileşimler (üç hücrelik komşuluk kuralları), belirlenimci olmasına rağmen nitel olarak çok farklı büyük ölçekli davranışlar üretebiliyor. Bu, hücresel otomatlarda belirme olgusunun somut bir örneği. Hafta 3, buna bir boyut daha ekliyor: aynı kural altında neredeyse aynı iki başlangıçtan yola çıkıldığında bile, kurala bağlı olarak bilgi hızla sönebiliyor, ışık konisi hızında yayılabiliyor ya da ikisi arasında kısmi bir davranış sergileyebiliyor.
